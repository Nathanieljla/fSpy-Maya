...
class Module_manager(QThread):
    ...
    def __init__(self, module_name, module_version, repo_name=''):
        ...
        if not self.repo_name:
            self.repo_name = self.module_name
...
class Custom_Installer(Module_manager):
    ...
    @staticmethod
    def pip_list(pip_path, pip_args = [], *args, **kwargs):
        cmd_str = ('{0}&list').format(pip_path)
        args = cmd_str.split('&') + pip_args
        stdout, stderr = Module_manager.run_shell_command(args, 'Listing Pip Packages')
        
        return stdout


    @staticmethod
    def pip_show(pip_path, repo_name, pip_args = [], *args, **kwargs):
        cmd_str = ('{0}&show&{1}').format(pip_path, repo_name)
        args = cmd_str.split('&') + pip_args
        stdout, stderr = Module_manager.run_shell_command(args, 'Show Pip Package Info')
        
        return stdout

    
    def get_pip_list(self, *args, **kwargs):
        result = Custom_Installer.pip_list(self.pip_path, *args, **kwargs)
        return result
        
    
    def get_pip_show(self, *args, **kwargs):
        result = Custom_Installer.pip_show(self.pip_path, self.repo_name, *args, **kwargs)
        return result
    
    
    def repo_installed(self):
        """returns True if the repo is already on the system"""
        return self.get_pip_list().find(self.repo_name) != -1
    
    
    def repo_outdated(self):
        """Check to see if a local install is outdated
        
        Checks to see if the repo is missing or out-of-date. The first
        element of the return tuple will always be True if the repo is
        outdated, else False. The first and second element of the tuple
        will only be valid if the repo is installed, but out-of-date
        tuple[1] = Tuple of installed version
        tuple[2] = Tuple of latest version
        
        
        Returns:
        --------
        tuple
            (bool:out-of-date, tuple:installed_version, tuple:latest_version)
        """
        if not self.repo_installed():
            return (True, None, None)
        
        result = self.get_pip_list(pip_args =['--outdated'])
        outdated = result.find(self.repo_name) != -1
    ...
    def get_install_cmd(self):
        ...
        #download the module
        #cmd_str = r'{0}&download&{1}&--no-deps&--dest={2}'.format(self.pip_path, self.get_repository(), r'C:\Users\natha\Documents\maya\modules\fSpy_maya\downloads')
            
        #1. use python_path to install the package to the non-maya module specific path
        #2. Use this to get the current pacakge version. ".\mayapy.exe -m pip show fspy-maya"
            
        package_local = r'C:\Users\natha\Documents\maya\modules\fSpy_maya\local_install'
        if package_local not in sys.path:
            sys.path.append(package_local)
        #List packages    
        cmd_str = ('{0}&list').format(self.pip_path)
        
        ##get package info
        #cmd_str = ('{0}&show&{1}').format(self.pip_path, 'SyncsketchGUI')
...
def main():
    if MAYA_RUNNING:
        manager = Custom_Installer('fSpy_maya', 1.0, repo_name = 'fspy-maya')
