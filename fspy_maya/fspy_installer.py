...
class Module_manager(object):
    ...
    def __init__(self, module_name, module_version):
        ...
        #python and pip pathes are needed for installing on linux and osX
        self.python_path = ''
        self.pip_path = ''
            
        self.version_specific = self.is_version_specific()    
    ...
    def define_paths(self):
        ...
        self.module_install_dir = os.path.join(self.scripts_dir, self.module_name)
...
class Resources(object):
    ...
    def company_icon(self):
        result = None
        if Resources.companu_base64:
            result = self.base64_to_QPixmap(Resources.companu_base64)
        
        return result
    
    @company_icon.setter
    def company_icon(self, value):
        Resources.companu_base64 = value
      
    
class Installer_UI(QWidget):
    def __init__(self, name, module_manager, *args, **kwargs):
    ...
    def create_layout(self):
        ...
        logo = None
        if RESOURCES.company_icon:
            logo = QLabel()
            smallLogo = RESOURCES.company_icon.scaled(240, 110, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    
            logo.setPixmap(smallLogo)
            logo.setAlignment(Qt.AlignCenter | Qt.AlignCenter)
            logo.setMargin(15)

        self.install_button = IconButton('Install', highlight=True)
        self.close_button = IconButton(' Close', icon=RESOURCES.install_icon)
...
def main():
    if MAYA_RUNNING:
        ...
        manager = Module_manager('fSpy_maya', 1.0)
        window_name =  'fSpy Maya'    
        
        #print('\nRunning {0} installer'.format(window_name))        
        
        #try:
            #maya.cmds.CG_3D_GURU_INSTALLER
            #print("\tdeleting old window")
            #maya.cmds.CG_3D_GURU_INSTALLER.deleteLater()
        #except:
            #pass
        ...
        installer = Installer_UI(window_name, manager)
